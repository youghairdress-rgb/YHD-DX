...
export function displayDiagnosisResult(result) {
    ...
    // 1. Skeleton
    const skContainer = document.getElementById('skeleton-result-content');
    if (skContainer && result.skeleton) {
        skContainer.innerHTML = '';
        skContainer.appendChild(createResultItem('顔型', result.skeleton.faceShape));
        // Only show valid features
        Object.entries(result.skeleton.features || {}).forEach(([k, v]) => {
            if (v && v !== 'N/A') skContainer.appendChild(createResultItem(k, v));
        });
    }

    // 2. Personal Color
    const pcContainer = document.getElementById('personal-color-result-content');
    if (pcContainer && result.personalColor) {
        pcContainer.innerHTML = '';
        pcContainer.appendChild(createResultItem('シーズン', result.personalColor.season));
        if (result.personalColor.bestColors && result.personalColor.bestColors.length) {
            const colors = result.personalColor.bestColors.join(', ');
            pcContainer.appendChild(createResultItem('ベストカラー', colors));
        }
    }

    // 3. Hair Condition
    const hcContainer = document.getElementById('hair-condition-result-content');
    if (hcContainer && result.hairCondition) {
        hcContainer.innerHTML = '';
        hcContainer.appendChild(createResultItem('髪質', result.hairCondition.texture));
        hcContainer.appendChild(createResultItem('現在のレベル', result.hairCondition.currentLevel));
        if (result.hairCondition.damageLevel) {
